...
class SSEClient {
   ...
   * @returns 关闭函数
   */
  connect(path: string, handlers: SSEHandlers, options: SSEOptions & { token?: string } = {}): () => void {
    const {
      reconnect = true,
      reconnectDelay = 3000,
      maxReconnect = 5,
    ...
    // SSE 路由在 backend 注册为 /sse/*（不带 /api/v1 前缀），所以不拼 API_BASE
    ...
    const open = () => {
      ...
      Object.entries(handlers).forEach(([event, fn]) => {
        if (event === 'error' || event === 'reconnect' || event === 'onEvent') return
      ...
      // 全局 onEvent 兜底（捕获任意事件名）
      const globalHandler = handlers.onEvent
      if (globalHandler) {
        // 监听常见事件名
        ;['connected', 'activity', 'alert', 'progress', 'item_done', 'summary', 'completed', 'keepalive'].forEach(name => {
          es!.addEventListener(name, (e: MessageEvent) => {
            try {
              const data = JSON.parse(e.data)
              globalHandler(name, data)
            } catch (err) {
              globalHandler(name, e.data)
            }
          })
        })
      }
