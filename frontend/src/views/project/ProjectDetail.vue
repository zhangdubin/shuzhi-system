...
const projectStatusMap: Record<string, { label: string; color: string }> = {
  ...
  finishing:   { label: '即将完成', color: 'warning' },
  completed:   { label: '已完成', color: 'success' },
  done:        { label: '已完成', color: 'success' },
  paused:      { label: '已暂停', color: 'info' },
  closed:      { label: '已关闭', color: 'gray' },
}
function normalizeProjectStatus(s: string): { label: string; color: string } {
  return projectStatusMap[(s || '').toLowerCase()] || { label: s || '未知', color: 'gray' }
}
function computePeriod(start: string, end: string): string {
  if (!start || !end) return '-'
...
function applyProjectData(d: any) {
  if (!d) return
  const st = normalizeProjectStatus(d.status)
  active.id = d.id || d.projectId || active.id
  active.code = d.code || active.code
  active.name = d.name || active.name
  active.status = st.label
  active.statusColor = st.color
  active.progress = Number(d.progress ?? d.completionRate ?? active.progress) || 0
  active.client = d.clientName || d.client || active.client
  active.type = d.type || active.type
  active.manager = d.managerName || d.manager || active.manager
  active.startDate = d.startDate || active.startDate
  active.endDate = d.endDate || active.endDate
  // budget 后端单位：分
  const budgetYuan = (Number(d.budget) || 0) / 100
  if (budgetYuan > 0) active.budget = budgetYuan.toLocaleString('zh-CN', { maximumFractionDigits: 0 })
  active.period = computePeriod(active.startDate, active.endDate)
  if (d.description) active.description = d.description
  if (Array.isArray(d.milestones) && d.milestones.length) {
    milestones.value = d.milestones.map((m: any) => ({
      seq: m.seq || m.id,
      name: m.name,
      date: m.plannedEnd || m.completedAt || m.date || '-',
      status: m.status || 'todo',
    }))
  }
}

// 4 关键统计（跟随 active 实时算）
const stats = computed(() => [
  { v: String(active.progress),       l: '整体进度', u: '%' },
  { v: '¥ ' + (active.budget || '0'), l: '预算总额', u: '' },
  { v: String(milestones.value.length), l: '里程碑',   u: '个' },
  { v: String(invoices.value.length),  l: '关联发票', u: '张' },
])
...
onMounted(() => {
  loading.value = true
  const id = Number(route.params.id) || active.id
  projectApi.detail(id)
    .then((res: any) => {
      const d = (res && (res.data || res.id)) ? (res.data || res) : null
      if (d) applyProjectData(d)
    })
    .catch(() => { /* 静默用 mock 兜底，不弹错 */ })
    ...
    <div class="project-hero">
      <div class="ph-top">
        <div class="ph-id">
          ...
          <span :class="['tag', `tag-${active.statusColor}`]">{{ active.status }}</span>
        </div>
        <h1 class="ph-title">{{ active.name }}</h1>
        <div class="ph-meta">
      ...
      <div class="ph-progress">
        <div class="ph-progress-label">
          <span>整体进度</span>
          <span class="pct">{{ active.progress }}%</span>
        </div>
        <div class="ph-bar">
          <div class="ph-fill" :style="{ width: active.progress + '%' }"></div>
        </div>
      </div>
    ...
    <div v-show="activeTab === 'basic'">
      ...
      <div class="detail-section">
        ...
        <div class="detail-section-body">
          <div class="info-grid">
            <div class="info-row"><span class="l">项目编号</span><span class="v mono">{{ active.code }}</span></div>
            <div class="info-row"><span class="l">项目类型</span><span class="v">{{ active.type }}</span></div>
            <div class="info-row"><span class="l">客户名称</span><span class="v">{{ active.client }}</span></div>
            <div class="info-row"><span class="l">项目经理</span><span class="v">{{ active.manager }}</span></div>
            <div class="info-row"><span class="l">开始日期</span><span class="v">{{ active.startDate }}</span></div>
            <div class="info-row"><span class="l">截止日期</span><span class="v">{{ active.endDate }}</span></div>
            <div class="info-row"><span class="l">预算总额</span><span class="v amount">¥ {{ active.budget }}</span></div>
            <div class="info-row"><span class="l">工期</span><span class="v">{{ active.period }}</span></div>
            <div class="info-row full">
              <span class="l">项目描述</span>
              <div class="notes-block">{{ active.description }}</div>
            </div>
          </div>
        </div>
      </div>
