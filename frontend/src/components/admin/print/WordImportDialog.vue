<!--
  WordImportDialog · Word 模板导入弹窗 (M3 阶段 4 下半)
  - 上传 .docx → 解析段落和表格为 schemaJson + HTML 预览
  - 命名 + docType → 保存为新模板
-->
<template>
  <el-dialog
  ...
  >
    ...
    <div v-if="step === 1" class="step">
      <p class="step-desc">选择 .docx 文件, 系统会自动解析段落和表格并生成 grid 模板。</p>
      <el-upload
        ref="uploadRef"
        :auto-upload="false"
        :limit="1"
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">仅支持 .docx 格式, 大小不超过 10MB</div>
        </template>
      </el-upload>
    ...
    <div v-else-if="step === 2" class="step">
      <div class="result-summary">
        ...
        <span>共 {{ preview?.totalElements }} 个元素 (段落 + 表格)</span>
      </div>
      <!-- 元素统计 -->
      <div class="element-stats">
        <el-tag size="small">段落: {{ paragraphCount }}</el-tag>
        <el-tag size="small">表格: {{ tableCount }}</el-tag>
        <el-tag size="small" type="info">grid rows: {{ totalGridRows }}</el-tag>
        <el-tag v-if="preview?.placeholders.length" size="small" type="success">
          占位符: {{ preview?.placeholders.length }}
        </el-tag>
      </div>
      <!-- 警告提示 -->
      <el-alert
      ...
      >
        <template #title>
          <div v-for="(warning, index) in preview?.warnings" :key="index">
            {{ warning }}
          </div>
        </template>
      </el-alert>
      <!-- 占位符提示 -->
      <el-alert
        v-if="preview?.placeholders.length"
        type="info"
        :closable="false"
        show-icon
        style="margin-bottom: 8px;"
      >
        检测到占位符: <code v-for="p in preview?.placeholders" :key="p" class="ph-code">{{ p }}</code>
      </el-alert>
      <!-- HTML 预览 -->
      <iframe
        v-if="preview?.html"
    ...
    <div v-else-if="step === 3" class="step">
      <el-form :model="form" label-width="100px" size="default">
        <el-form-item label="模板 code" required>
          <el-input v-model="form.code" placeholder="例：contract_docx_v1" />
        </el-form-item>
        <el-form-item label="模板名称" required>
          <el-input v-model="form.name" placeholder="例：合同模板（Word 导入）" />
        </el-form-item>
...
import { docxImportApi, type DocxImportPreview } from '@/api/print'
...
const preview = ref<DocxImportPreview | null>(null)
const uploadRef = ref<UploadInstance>()

const paragraphCount = computed(() => {
  if (!preview.value) return 0
  return preview.value.schemaJson.body.filter((b: any) => b.type === 'title' || b.type === 'text').length
})

const tableCount = computed(() => {
  if (!preview.value) return 0
  return preview.value.schemaJson.body.filter((b: any) => b.type === 'grid').length
})

const totalGridRows = computed(() => {
  if (!preview.value) return 0
  let count = 0
  for (const block of preview.value.schemaJson.body) {
    if (block.type === 'grid' && block.rows) {
...
async function parseFile() {
  ...
  errorMsg.value = null
  try {
    const r = await docxImportApi.preview(selectedFile.value.raw as File)
    preview.value = r
    step.value = 2
  } catch (e: any) {
...
function goSave() {
  ...
  if (!form.name) {
    form.name = `${preview.value.filename.replace('.docx', '')} (Word 导入)`
  }
  step.value = 3
}
...
async function save() {
  ...
  saving.value = true
  try {
    const t = await docxImportApi.confirm({
      code: form.code,
      name: form.name,
      docType: form.docType,
      paper: form.paper,
      orientation: form.orientation,
      schemaJson: preview.value.schemaJson,
